...
local Rayfield = loadstring(game:HttpGet(
                                'https://raw.githubusercontent.com/Helloss794/rayfield/refs/heads/main/rayfield.lua'))()

local Window = Rayfield:CreateWindow({
    Name = string.format(
        "NewNoobie All Star Tower Defense Script (Version %s)", version),
    LoadingTitle = "All Star Tower Defense Script",
    LoadingSubtitle = "by NewNoobie HUB",
    LoadingVersion = string.format("Version %s", version),
    ConfigurationSaving = {Enabled = false},
    Discord = {Enabled = true, Invite = "Ktu7ZwaM", RememberJoins = true},
    KeySystem = false
})

print("[NewNoobie] Rayfield Loaded: " .. os.clock() - benchmark_time)
benchmark_time = os.clock()

if not isfolder("NewNoobie") then makefolder("NewNoobie") end

if not isfolder("NewNoobie\\ASTD") then makefolder("NewNoobie\\ASTD") end

if not isfolder("NewNoobie\\ASTD\\Settings") then
    makefolder("NewNoobie\\ASTD\\Settings")
end

local SettingsFile = "NewNoobie\\ASTD\\Settings\\" ..
...
local folder_name = "NewNoobie\\ASTD\\" .. game.Players.LocalPlayer.UserId
...
Save()
print("[NewNoobie] Filesystem Loaded: " .. os.clock() - benchmark_time)
benchmark_time = os.clock()
                ...
                ["embeds"] = {
                    {
                        ["author"] = {
                            ...
                            ["icon_url"] = "https://raw.githubusercontent.com/Helloss794/rayfield/main/logo.png"
                        },
...
local function importSettings(url)
    local fileName = extractFileName(url)
    local tempSavePath = "NewNoobie\\ASTD\\Settings\\" .. fileName
    local success, response = pcall(function() return game:HttpGet(url) end)
...
print("[NewNoobie] Functions Loaded: " .. os.clock() - benchmark_time)
benchmark_time = os.clock()
...
function InitializeUI()
    ...
    local function MiscellaneousSettings()
        ...
        Miscellaneous:CreateButton({
            ...
            Callback = function()
                ...
                task.wait(2)
                if isfile("NewNoobie\\key.panda") then
                    library.LoadScript(readfile("NewNoobie\\key.panda"))
                end
            end,
    ...
    local function CreditsSettings()
        ...
        Credits:CreateParagraph({
            Title = "Made by NewNoobie & Maintained by NewNoobie",
            Content = "Credit by #zding_2020 by Team NewNoobie HUB",
...
print("[NewNoobie] UI Loaded: " .. os.clock() - benchmark_time)
benchmark_time = os.clock()
...
if Settings.auto_execute then
    ...
    if not _G.auto_executed then
        ...
        print("[NewNoobie]: Queue on teleport for auto execute sucessful!")
    end
end
